...
cat $usedir/phone.txt | sort -u > $dir/phones.txt
#paste $dir/phones.txt $dir/phones.txt > $dir/lexicon1.txt || exit 1;
cp $usedir/lexicon.txt $dir/lexicon1.txt
grep -v -F -f $dir/silence_phones.txt $dir/phones.txt > $dir/nonsilence_phones.txt 
(echo '!sil sil'; echo '<noise> spn'; echo '<unk> spn' ; echo '** spn' ; echo '<FIL/> spn'; echo '<SPK/> spn' ; echo '<NON/> spn' ; echo '<STA/> spn'; echo '<NPS/> spn' ) | cat - $dir/lexicon1.txt > $dir/lexicon.txt
...
cp $usedir/new.lm.little $tmpdir/lm.arpa
...
#
#cp $data_all_dir/wav1.scp $data_all_dir/utt2spk
#cp $data_all_dir/utt2spk $data_all_dir/spk2utt
#cp $data_all_dir/utt2spk $data_all_dir/wav.scp

rm -rf data/train
rm -rf data/test
utils/subset_data_dir_tr_cv.sh --cv-utt-percent 1 data/data_all data/train data/test
utils/subset_data_dir.sh --first data/train  80000 data/train_120h

utils/subset_data_dir.sh --first data/test  600  data/test_600
